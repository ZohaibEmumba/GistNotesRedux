...
export default class GithubProfile extends Component {
    ...
    let authData = await loginAuthUser(userName).then(data => this.setState({ authUserRecord: data }));
    }
    getGists = async () => {
    const getAuthGists = await privateGistsRecord().then(data => this.setState({gists : data}))
      }
      ...
      <section className="whole-section">
        <div className="profile-left">
          <div style={{display:'flex' , justifyContent:"center"}}>
            <img id="profile-pic" src={authUserRecord?.avatar_url} alt="zohaib" />
          </div>
          <div id="heading-name" style={{display:'flex' , justifyContent:"center"}}>
           ...
           gists.map((item , index) =>
            ...
            <div className="left-sec">
              <div className="profile-col">
                ...
                <div>
                  <span className="">
                    <h4>{item?.owner?.login}/{Object.keys(item?.files)[0]}</h4>
            ...
            <section id="content-body">
              <div className="card-body">
                ...
                <span className="file-name">{"  "} {Object.keys(item?.files)[0]}</span>
              </div>
